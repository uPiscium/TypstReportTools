  ...
  if func != none or class != none {
    ...
    for l in txt {
      ...
      if (funcName in l) and index == none {
        ...
        not (regex("^\s*$") in l) and index != none and (spaceNum(l) > index)
      ) {
        end = count
      } else if (
        not (regex("^\s*$") in l) and index != none and (spaceNum(l) <= index)
...
#let box(caption, it) = { itembox(caption: caption)[ #it ] }
#let exbox(it) = { box("課題内容", it) }
